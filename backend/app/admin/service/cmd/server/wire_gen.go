...
func initApp(logger log.Logger, registrar registry.Registrar, bootstrap *v1.Bootstrap) (*kratos.App, func(), error) {
	...
	inSiteMessageRepo := data.NewInSiteMessageRepo(dataData, logger)
	inSiteMessageService := service.NewInSiteMessageService(inSiteMessageRepo, logger)
	inSiteMessageCategoryRepo := data.NewInSiteMessageCategoryRepo(dataData, logger)
	inSiteMessageCategoryService := service.NewInSiteMessageCategoryService(inSiteMessageCategoryRepo, logger)
	httpServer := server.NewRESTServer(bootstrap, logger, authenticator, engine, userToken, adminOperationLogRepo, adminLoginLogRepo, authenticationService, userService, menuService, routerService, organizationService, roleService, positionService, dictService, departmentService, adminLoginLogService, adminOperationLogService, ossService, uEditorService, fileService, inSiteMessageService, inSiteMessageCategoryService)
