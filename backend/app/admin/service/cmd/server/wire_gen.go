...
func initApp(logger log.Logger, registrar registry.Registrar, bootstrap *v1.Bootstrap) (*kratos.App, func(), error) {
	...
	dictMainRepo := data.NewDictMainRepo(dataData, logger)
	dictItemRepo := data.NewDictItemRepo(dataData, logger)
	dictService := service.NewDictService(logger, dictMainRepo, dictItemRepo)
