...
INSERT INTO kratos_admin.public.sys_tenants(id, name, code, type, audit_status, status, admin_user_id, create_time)
VALUES
...
-- 主字典
TRUNCATE TABLE kratos_admin.public.sys_dict_mains CASCADE;
ALTER SEQUENCE kratos_admin.public.sys_dict_mains_id_seq RESTART WITH 1;
INSERT INTO kratos_admin.public.sys_dict_mains(id, code, name, sort_id, remark, status, create_time)
VALUES
    (1, 'USER_STATUS', '用户状态', 10, '系统用户的状态管理，包括正常、冻结、注销', 'ON', now()),
    (2, 'DEVICE_TYPE', '设备类型', 20, 'IoT平台接入的设备品类，新增需同步至设备接入模块', 'ON', now()),
    (3, 'ORDER_STATUS', '订单状态', 30, '电商订单的全生命周期状态', 'ON', now()),
    (4, 'GENDER', '性别', 40, '用户性别枚举，默认未知', 'ON', now()),
    (5, 'PAYMENT_METHOD', '支付方式', 50, '支持的支付渠道，含第三方支付和自有渠道', 'ON', now())
;
SELECT setval('sys_dict_mains_id_seq', (SELECT MAX(id) FROM sys_dict_mains));

-- 子字典
TRUNCATE TABLE kratos_admin.public.sys_dict_items RESTART IDENTITY;
INSERT INTO kratos_admin.public.sys_dict_items(id, main_id, code, name, value, sort_id, remark, status, create_time)
VALUES
    -- 用户状态
    (1, 1, 'NORMAL', '正常', 1, 1, '用户可正常登录和操作', 'ON', now()),
    (2, 1, 'FROZEN', '冻结', 2, 2, '因违规被临时冻结，需管理员解冻', 'ON', now()),
    (3, 1, 'CANCELED', '注销', 3, 3, '用户主动注销，数据保留但不可登录', 'ON', now()),
    -- 设备类型
    (4, 2, 'TEMP_SENSOR', '温湿度传感器', 101, 1, '支持温度（-20~80℃）和湿度（0~100%RH）采集', 'ON', now()),
    (5, 2, 'CURRENT_METER', '电流仪表', 102, 2, '交流/直流电流测量，精度0.5级', 'ON', now()),
    (6, 2, 'GAS_DETECTOR', '气体探测器', 103, 3, '暂不支持，待硬件适配（2025Q4计划启用）', 'OFF', now()),
    -- 订单状态
    (7, 3, 'PENDING', '待支付', 1, 1, '下单后未支付，超时自动取消', 'ON', now()),
    (8, 3, 'PAID', '已支付', 2, 2, '支付成功，等待发货', 'ON', now()),
    (9, 3, 'SHIPPED', '已发货', 3, 3, '商品已出库，物流配送中', 'ON', now()),
    (10, 3, 'COMPLETED', '已完成', 4, 4, '用户确认收货，订单结束', 'ON', now()),
    (11, 3, 'CANCELED', '已取消', 5, 5, '用户或系统取消订单', 'ON', now()),
    -- 性别
    (12, 4, 'MALE', '男', 1, 1, '', 'ON', now()),
    (13, 4, 'FEMALE', '女', 2, 2, '', 'ON', now()),
    (14, 4, 'UNKNOWN', '未知', 0, 3, '用户未填写时默认值', 'ON', now()),
    -- 支付方式
    (15, 5, 'ALIPAY', '支付宝', 1, 1, '支持花呗、余额宝', 'ON', now()),
    (16, 5, 'WECHAT', '微信支付', 2, 2, '需绑定微信', 'ON', now()),
    (17, 5, 'UNIONPAY', '银联支付', 3, 3, '支持信用卡、储蓄卡', 'ON', now()),
    (18, 5, 'CASH', '现金支付', 4, 4, '线下支付，已废弃（2025-01停用）', 'OFF', now())
;
SELECT setval('sys_dict_items_id_seq', (SELECT MAX(id) FROM sys_dict_items));
